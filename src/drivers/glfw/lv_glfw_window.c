...
static void window_update_handler(lv_timer_t* t) {
  ...
  LV_LL_READ(&glfw_window_ll, window) {
    ...
    lv_glfw_texture_t* texture;
    LV_LL_READ(&window->textures, texture) {
      /* if the added texture is an LVGL opengles texture display, refresh it
       * before rendering it */
      lv_display_t* texture_disp =
          lv_opengles_texture_get_from_texture_id(texture->texture_id);
      if (texture_disp != NULL) {
        lv_refr_now(texture_disp);
      }

      lv_opengles_render_texture(texture->texture_id, &texture->area,
                                 texture->opa, window->hor_res,
                                 window->ver_res);
    }
...
static void mouse_button_callback(GLFWwindow* window,
  ...
  if (button == GLFW_MOUSE_BUTTON_LEFT) {
    lv_glfw_window_t* lv_window = lv_glfw_get_lv_window_from_window(window);
    lv_window->mouse_last_state =
        action == GLFW_PRESS ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED;
...
static void proc_mouse(lv_glfw_window_t* window) {
  ...
  LV_LL_READ_BACK(&window->textures, texture) {
    if (lv_area_is_point_on(&texture->area, &window->mouse_last_point, 0)) {
      /* adjust the mouse pointer coordinates so that they are relative to the
       * texture */
      texture->indev_last_point.x =
          window->mouse_last_point.x - texture->area.x1;
      texture->indev_last_point.y =
